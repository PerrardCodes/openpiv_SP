...
def extended_search_area_piv( np.ndarray[DTYPEi_t, ndim=2] frame_a, 
    ...
    if search_area_size[0] == 0 or search_area_size[1] == 0:
        search_area_size = window_size
    
    if overlap[0] >= window_size[0] and overlap[1] >= window_size[1]:
        raise ValueError('Overlap has to be smaller than the window_size')
    
    if search_area_size[0] < window_size[0] and search_area_size[1] < window_size[1]:
        raise ValueError('Search size cannot be smaller than the window_size')
    
        
    if (window_size[0] > frame_a.shape[0]) or (window_size[1] > frame_a.shape[1]):
    ...
    # define arrays
    cdef np.ndarray[DTYPEi_t, ndim=2] window_a = np.zeros([window_size[0], window_size[1]], dtype=DTYPEi)
    cdef np.ndarray[DTYPEi_t, ndim=2] search_area = np.zeros([search_area_size[0], search_area_size[1]], dtype=DTYPEi)
    cdef np.ndarray[DTYPEf_t, ndim=2] corr = np.zeros([search_area_size[0], search_area_size[1]], dtype=DTYPEf)
    ...
    I = 0
    for i in range( 0, frame_a.shape[0]-window_size[0]+1, window_size[0]-overlap[0]):
        J = 0
        for j in range( 0, frame_a.shape[1]-window_size[1]+1, window_size[1]-overlap[1]):
            ...
            for k in range( search_area_size[0]):
                for l in range( search_area_size[1]):
                    
                    # fill with zeros if we are out of the borders
                    if i+window_size[0]/2-search_area_size[0]//2+k < 0 or \
                        i+window_size[0]//2-search_area_size[0]//2+k >= frame_b.shape[0]:
                        search_area[k,l] = 0
                    elif j+window_size[1]//2-search_area_size[1]//2+l < 0 or \
                        j+window_size[1]//2-search_area_size[1]//2+l >= frame_b.shape[1]:
                        search_area[k,l] = 0
                    else:
                        search_area[k,l] = frame_b[ i+window_size[0]//2-search_area_size[0]//2+k,
                            j+window_size[1]//2-search_area_size[1]//2+l ]
            ...
            if any(window_a.flatten()):
                ...
                # velocities
                v[I,J] = -( (i_peak - corr.shape[0]/2) - (search_area_size[0]-window_size[0])/2 ) / dt
                u[I,J] =  ( (j_peak - corr.shape[1]/2) - (search_area_size[1]-window_size[1])/2 ) / dt
...
def get_field_shape ( image_size, window_size, overlap ):
    ...
    return ( (image_size[0] - window_size[0])//(window_size[0]-overlap[0])+1, 

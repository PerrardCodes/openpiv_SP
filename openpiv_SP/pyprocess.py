...
def get_coordinates(image_size, window_size, overlap):
    ...
    x = np.arange(field_shape[1]) * (window_size[0] -
                                     overlap[0]) + (window_size[0] - 1) / 2.0
    y = np.arange(field_shape[0]) * (window_size[1] -
                                     overlap[1]) + (window_size[1] - 1) / 2.0
...
def moving_window_array(array, window_size, overlap):
    ...
    strides = (sz * shape[0] * (window_size[0] - overlap[0]),
               sz * (window_size[1] - overlap[1]), sz * shape[1], sz)
    shape = (int((shape[0] - window_size[0]) / (window_size[0] - overlap[0])) + 1, int(
        (shape[1] - window_size[1]) / (window_size[1] - overlap[1])) + 1, window_size[0], window_size[1])

    return numpy.lib.stride_tricks.as_strided(array, strides=strides, shape=shape).reshape(-1, window_size[0], window_size[1])
...
def sig2noise_ratio(corr, sig2noise_method='peak2peak', width=2):
    ...
    if sig2noise_method == 'peak2peak':
        ...
        # wrong
        if corr_max1 < 1e-3 or (peak1_i == 0 or peak1_i == corr.shape[0] or peak1_j == 0 or peak1_j == corr.shape[1] or
                                peak2_i == 0 or peak2_i == corr.shape[0] or peak2_j == 0 or peak2_j == corr.shape[1]):
